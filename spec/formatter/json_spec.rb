...
RSpec.describe Logist::Formatter::Json do
  ...
  context "message is a string" do
    ...
    it do
      expect(deserialized_output).to eq('level' => 'debug', 'environment' => 'development', 'timestamp' => now.strftime("%Y-%m-%dT%H:%M:%S.%6N "), 'message' => 'test message')
    end
  end
  ...
  context "message is a json string" do
    ...
    it do
      expect(deserialized_output).to eq('level' => 'debug', 'environment' => 'development', 'timestamp' => now.strftime("%Y-%m-%dT%H:%M:%S.%6N "), 'hoge' => 'fuga')
    end
  end
end
